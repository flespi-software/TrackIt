<template>
    <q-item
      ...
      :class="[isSelected ? 'device-item__selected' : '', device['x-flespi-no-access'] ? 'no-access-device' : '']"
      ...
      <q-item-section>
        <q-item-label class="ellipsis-2-lines q-pa-none text-white" header>{{device.name || '&lt;noname&gt;'}}</q-item-label>
      </q-item-section>
...
export default {
  ...
  methods: {
    deviceClickHandler () {
      ...
      this.$emit('device-in-devices-list-ckick', true)
    },
